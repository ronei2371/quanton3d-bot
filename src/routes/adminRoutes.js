...
function buildAdminRoutes(adminConfig = {}) {
  ...
  router.post("/suggest-knowledge", async (req, res) => {
    try {
      ...
      const { title, content, suggestion, tags, source, userName, userPhone, lastUserMessage, lastBotReply } = req.body || {};
      const normalizedTitle = (title || suggestion || "Sugestão do cliente").toString().trim();
      const normalizedContent = (content || suggestion || "").toString().trim();

      if (!normalizedContent) {
        return res.status(400).json({ success: false, error: "Sugestão é obrigatória" });
      }
      ...
      console.log(`✅ [ADMIN] Nova sugestão criada: ${normalizedTitle}`);

...
codex/revert-changes-to-apiroutes.js-and-adminpanel.jsx-9pxszf
...
router.post("/gallery", multipartUpload.any(), async (req, res) => {
  try {
    const {
      name,
      resin,
...
router.post('/visual-knowledge', multipartUpload.any(), async (req, res) => {
  try {
    const mongoReady = await ensureMongoReady();
    if (!mongoReady) {
      return res.status(503).json({ success: false, error: 'Banco de dados indisponivel' });
...
router.post('/partners/upload-image', multipartUpload.any(), async (req, res) => {
  try {
    if (!isValidAdminToken(req)) {
      return res.status(401).json({ success: false, error: 'unauthorized' });
    }
